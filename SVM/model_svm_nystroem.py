...
# Linear SVC pipeline with Nystroem Approximation
# 1. StandardScaler: Critical for SVM convergence
# 2. Nystroem: Approximates RBF kernel for non-linear separation
# 3. LinearSVC: Fast linear solver
print("Initializing LinearSVC model with Nystroem approximation...")

# n_components=300 is a balance between speed and accuracy (default is 100)
...
print(f"\n{'='*60}")
print(f"MODEL EVALUATION RESULTS (Nystroem + LinearSVC)")
print(f"{'='*60}")
...
version = 1
while os.path.exists(f'svm_nystroem_wetland_model_v{version}.pkl'):
    version += 1

# Create filename with version + timestamp
timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
model_filename = f'svm_nystroem_wetland_model_v{version}_{timestamp}.pkl'
metadata_filename = f'svm_nystroem_wetland_model_v{version}_{timestamp}_metadata.json'
